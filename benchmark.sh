...
# Base humaneval (not humaneval_instruct): do NOT pass --apply_chat_template — that
# breaks code stitching and yields pass@1=0 on instruct models.
...
if [[ -d "${MODEL_ARG}" ]]; then
    ...
    OUT_DIR=${2:-"${SCRIPT_DIR}/benchmark_results/${SAFE_NAME}"}
    OUT_DIR="$(mkdir -p "${OUT_DIR}" && cd "${OUT_DIR}" && pwd)"
