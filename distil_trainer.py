...
@torch.no_grad()
def compute_q_star_logps(log_p, log_T, rho, n_iter=20):
    """Info-Proximal SDFT target distribution q*.

    Solve per token:
        q*_t = argmin_q KL(q||p_t)  s.t.  E_q[log T_t / p_t] >= rho * KL(T_t||p_t)

    as lambda_t in [0, 1] is we find it by bisection

    Args:
        log_p: log-probs of the student p_t, shape [B, T, V].
        log_T: log-probs of the teacher T_t, shape [B, T, V].
        rho: fraction of teacher local information to transfer, scalar in [0, 1].
        n_iter: number of bisection iterations.

    Returns:
        log_q_star: log-probs of q*_t, shape [B, T, V], detached.
        lam: Lagrange multiplier lambda_t, shape [B, T].
        gamma: target info budget gamma_t = rho * KL(T||p), shape [B, T].
        kl_T_p: KL(T||p) per token, shape [B, T].
    """
    log_p = log_p.detach().float()
    log_T = log_T.detach().float()

    delta = log_T - log_p
    kl_T_p = (log_T.exp() * delta).sum(-1)

    # gamma is the info budget per token. rho=1 -> full teacher info, rho=0 -> none.
    # unsqueeze adds a vocab dim so it broadcasts against [B, T, V] tensors below.
    gamma = (rho * kl_T_p).unsqueeze(-1)

    lo = torch.zeros_like(gamma)
    hi = torch.ones_like(gamma)
    for _ in range(n_iter):
        # try the midpoint of the current bracket
        lam = 0.5 * (lo + hi)

        log_q = (1.0 - lam) * log_p + lam * log_T
        log_q = log_q - torch.logsumexp(log_q, dim=-1, keepdim=True)

        G = (log_q.exp() * delta).sum(-1, keepdim=True)

        # since G is monotone in lambda, this always converges.
        too_low = (G < gamma)
        lo = torch.where(too_low, lam, lo)
        hi = torch.where(too_low, hi, lam)

    lam = 0.5 * (lo + hi)
    log_q_star = (1.0 - lam) * log_p + lam * log_T
    log_q_star = log_q_star - torch.logsumexp(log_q_star, dim=-1, keepdim=True)
...
class DistilTrainer(BaseTrainer):
    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.alpha == 0: # forward KL: KL(q* || p)
            ...
            kl_loss = kl_div(log_q_star, all_logps, reduction="none", log_target=True)
        else:
            # Compute the log of the mixture distribution
            # log(a + b) = log(exp(log(a)) + exp(log(b))) -> for mixture
            alpha = torch.tensor(self.alpha, dtype=all_logps.dtype)
            mixture_log_probs = torch.logsumexp(
                torch.stack([all_logps + torch.log(1 - alpha), log_q_star + torch.log(alpha)]),
                dim=0,
            )

            kl_teacher = kl_div(mixture_log_probs, log_q_star, reduction="none", log_target=True)
        ...
        # info-proximal SDFT diagnostics
        with torch.no_grad():
            log_q_star_f = log_q_star.float()
            all_logps_f = all_logps.detach().float()
            kl_qstar_p = (log_q_star_f.exp() * (log_q_star_f - all_logps_f)).sum(-1)  # KL(q* || p)
            mean_lam = masked_batch_mean(lam_t)
